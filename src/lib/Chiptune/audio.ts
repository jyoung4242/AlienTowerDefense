...
export function changeGain(newGain: number) {
...
document.addEventListener("gainChange", e => {
  gainFactor = (e as CustomEvent).detail;
});
...
function Audio(ctx: AudioContext) {
  ...
  function SquareSynth(pan: number = 0): Synth<Note> {
    const set = (a: AudioParam, v: number) => {
      ...
      if (a == gain) {
        a.setValueAtTime(v * gainFactor, ctx.currentTime);
      } else a.setValueAtTime(v, ctx.currentTime);
    };
    const towards = (a: AudioParam, v: number, t: number) => {
      if (a == gain) a.setValueAtTime(v * gainFactor, ctx.currentTime + t);
      else a.setTargetAtTime(t, ctx.currentTime, t);
    };
    ...
    function play(slot: Drum) {
      ...
      if (slot.drum === "KCK") {
        ...
        toneGain.gain.setValueCurveAtTime(
          new Float32Array([0.2 * vel * gainFactor, 0.2 * vel * gainFactor, 0.13 * vel * gainFactor, 0.05 * vel * gainFactor, 0.0]),
          ctx.currentTime,
          0.1
        );
        ...
        noiseGain.gain.setValueCurveAtTime(
          new Float32Array([0.1 * vel * gainFactor, 0.04 * vel * gainFactor, 0.0]),
          ctx.currentTime,
          0.08
        );
        ...
        toneGain.gain.setValueCurveAtTime(
          new Float32Array([0.15 * vel * gainFactor, 0.05 * vel * gainFactor, 0.01 * vel * gainFactor, 0]),
          ctx.currentTime,
          0.1
        );
        noiseGain.gain.cancelScheduledValues(ctx.currentTime);
        ...
        noiseGain.gain.setValueCurveAtTime(
          new Float32Array([0.2 * vel * gainFactor, 0.15 * vel * gainFactor, 0.0]),
          ctx.currentTime,
          0.15
        );
      }
